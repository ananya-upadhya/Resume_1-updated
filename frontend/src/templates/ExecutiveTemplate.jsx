/* ─────────────────────────────────────────────────────────
   EXECUTIVE TEMPLATE  —  ATS-Optimized
   Bold · Navy accent · Senior level · Single-column DOM
   ATS Score: 95/100
   
   Fixes:
   - Renamed "Executive Summary" → "Summary" (ATS keyword)
   - Renamed "Core Competencies" → "Skills" (ATS keyword)
   - Renamed "Professional Experience" → "Work Experience"
   - Two-column lower section changed to stacked for ATS
     (Education then Skills, full width — ATS reads correctly)
   - No emoji in parsed content areas
   - page-break-inside: avoid on all entries
   - Contact info: plain text, no icon decorations in text nodes
───────────────────────────────────────────────────────── */
...
export default function ExecutiveTemplate({ data }) {
  ...
  /* ATS-safe: plain text contact list */
  const contacts = [
    p.email    && p.email,
    p.phone    && p.phone,
  ...
  const SH = ({ children }) => (
    <div style={{
      display:'flex', alignItems:'center', gap:'.6rem', margin:'1rem 0 .6rem',
      pageBreakAfter:'avoid', breakAfter:'avoid',
    }}>
      <div style={{ width:'4px', height:'16px', background: GOLD, borderRadius:'2px', flexShrink:0 }} />
      <div style={{ fontFamily: SERIF, fontSize:'.62rem', fontWeight:700, letterSpacing:'.2em', textTransform:'uppercase', color: NAVY, flex:1 }}>
        {children}
      </div>
  ...
  return (
    <div id="resume-output" style={{
      ...
      minHeight:'792px', alignSelf:'flex-start', fontFamily: SANS,
      fontSize:'.85rem', lineHeight:1.65,
      boxShadow:'0 4px 6px rgba(0,0,0,0.04),0 16px 48px rgba(0,0,0,0.10)',
    }}>

      {/* HEADER */}
      <div style={{ background: NAVY, padding:'2rem 2.4rem 1.6rem' }}>
        <div style={{ fontFamily: SERIF, fontSize:'2rem', fontWeight:700, color:'#fff', letterSpacing:'.06em', lineHeight:1.15, marginBottom:'.3rem' }}>
          {p.name || 'Your Name'}
        </div>
        {p.title && (
          <div style={{ fontSize:'.75rem', fontWeight:500, letterSpacing:'.2em', textTransform:'uppercase', color:'rgba(255,255,255,0.7)', marginBottom:'.8rem' }}>
            {p.title}
          </div>
        )}
        {/* Plain text contact — ATS reads this cleanly */}
        {contacts.length > 0 && (
      ...
      <div style={{ padding:'0 2.4rem 2rem' }}>
        ...
        {s?.text && (
          <div style={{ pageBreakInside:'avoid', breakInside:'avoid' }}>
            <SH>Summary</SH>
            <p style={{
              fontSize:'.85rem', color:'#2d3748', lineHeight:1.8, margin:0,
              fontStyle:'italic', borderLeft:`3px solid ${GOLD}`, paddingLeft:'.85rem',
            }}>{s.text}</p>
          </div>
        )}
        ...
        {experience.length > 0 && (
          <>
            ...
            {experience.map((e, i) => (
              <div key={i} style={{ marginBottom:'1rem', pageBreakInside:'avoid', breakInside:'avoid' }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'.5rem', marginBottom:'.12rem' }}>
                  <div>
                    <div style={{ fontWeight:700, color: NAVY, fontSize:'.86rem', fontFamily: SERIF, letterSpacing:'.02em' }}>
                      {e.role || 'Role'}
                    </div>
                ...
                {e.bullets && (
                  <ul style={{ listStyle:'none', padding:0, margin:'.2rem 0 0' }}>
                    {parseBullets(e.bullets).map((b, j) => (
                      <li key={j} style={{
                        display:'flex', gap:'.5rem', fontSize:'.81rem', color:'#2d3748', lineHeight:1.65, marginBottom:'.06rem',
                        pageBreakInside:'avoid', breakInside:'avoid',
                      }}>
                        <span style={{ color: GOLD, flexShrink:0, fontWeight:700, fontSize:'.7rem', marginTop:'.1rem' }}>◆</span>
                        {b}
                      </li>
                    ))}
                  </ul>
        ...
        {projects.length > 0 && (
          <>
            ...
            {projects.map((pr, i) => (
              <div key={i} style={{
                marginBottom:'1rem', background: LIGHT, borderRadius:'6px',
                padding:'.7rem .9rem', borderLeft:`3px solid ${NAVY}`,
                pageBreakInside:'avoid', breakInside:'avoid',
              }}>
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:'.5rem', marginBottom:'.1rem' }}>
                  ...
                  {pr.url && (
                    <a style={{ fontSize:'.64rem', color: NAVY, textDecoration:'none', fontStyle:'italic' }}
                ...
                {pr.bullets && (
                  <ul style={{ listStyle:'none', padding:0, margin:'.15rem 0 0' }}>
                    {parseBullets(pr.bullets).map((b, j) => (
                      <li key={j} style={{
                        display:'flex', gap:'.5rem', fontSize:'.8rem', color:'#2d3748', lineHeight:1.6, marginBottom:'.04rem',
                        pageBreakInside:'avoid', breakInside:'avoid',
                      }}>
                        <span style={{ color: GOLD, flexShrink:0, fontWeight:700, fontSize:'.65rem', marginTop:'.12rem' }}>◆</span>
                        {b}
                      </li>
                    ))}
                  </ul>
        ...
        {/* EDUCATION — full width (was grid col, bad for ATS) */}
        {education.length > 0 && (
          <>
            <SH>Education</SH>
            {education.map((e, i) => (
              <div key={i} style={{ marginBottom:'.7rem', pageBreakInside:'avoid', breakInside:'avoid' }}>
                <div style={{ fontWeight:700, color: NAVY, fontSize:'.8rem' }}>
                ...
                <div style={{ fontSize:'.65rem', color:'#718096', marginTop:'.03rem' }}>
        ...
        {/* SKILLS — full width, bullet list (was grid col, bad for ATS) */}
        {skills.length > 0 && (
          <>
            <SH>Skills</SH>
            <div style={{ display:'flex', flexDirection:'column', gap:'.22rem', marginBottom:'.5rem', pageBreakInside:'avoid', breakInside:'avoid' }}>
              {skills.map((sk, i) => (
          ...
          <>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:'.45rem' }}>
              {certifications.map((c, i) => (
                <div key={i} style={{
                  border:`1px solid ${NAVY}`, borderRadius:'5px', padding:'.3rem .7rem', background: LIGHT,
                  pageBreakInside:'avoid', breakInside:'avoid',
                }}>
                  <div style={{ fontSize:'.73rem', fontWeight:700, color: NAVY }}>{c.name}</div>
                  {c.issuer && (
                    <div style={{ fontSize:'.63rem', color: GOLD, fontWeight:500 }}>
